...
use tari_wallet::crypto::*;
use tari_wallet::data_structures::address::{TariAddress, TariAddressFeatures};
use tari_wallet::data_structures::{Network, PrivateKey};
use tari_wallet::errors::*;
use tari_wallet::key_management::*;
use tari_wallet::wallet::*;

#[cfg(feature = "storage")]
use tari_wallet::storage::*;
...
async fn test_complete_wallet_creation_workflow() {
    ...
    assert!(single_address.features().contains(TariAddressFeatures::INTERACTIVE_ONLY));
    
    // Verify deterministic generation
    let dual_address_2 = wallet.get_dual_address(
        TariAddressFeatures::create_interactive_and_one_sided(),
        None,
    ...
    let reconstructed_dual = reconstructed_wallet.get_dual_address(
        TariAddressFeatures::create_interactive_and_one_sided(),
        None,
...
async fn test_wallet_creation_entropy_sources() {
    ...
    let encrypted_bytes = cipher_seed.encipher(Some("test_passphrase"))
        .expect("Failed to encrypt cipher seed");
    let seed_with_passphrase = bytes_to_mnemonic(&encrypted_bytes)
        .expect("Failed to convert to mnemonic");
    ...
    assert_eq!(wallet_with_passphrase.export_seed_phrase().unwrap(), seed_with_passphrase);
...
async fn test_wallet_metadata_workflow() {
    let mut wallet = Wallet::generate_new_with_seed_phrase(None)
...
async fn test_wallet_key_derivation_consistency() {
    ...
    let wallet1 = Wallet::new_from_seed_phrase(&seed_phrase, None)
        .expect("Failed to create wallet 1");
    let wallet2 = Wallet::new_from_seed_phrase(&seed_phrase, None)
    ...
    let addr1_dual = wallet1.get_dual_address(
        TariAddressFeatures::create_interactive_and_one_sided(),
        None,
    ).expect("Failed to generate dual address from wallet 1");
    
    let addr2_dual = wallet2.get_dual_address(
        TariAddressFeatures::create_interactive_and_one_sided(),
        None,
    ...
    let addr1_single = wallet1.get_single_address(
        TariAddressFeatures::create_one_sided_only()
    ).expect("Failed to generate single address from wallet 1");
    
    let addr2_single = wallet2.get_single_address(
        TariAddressFeatures::create_one_sided_only()
    ...
    let addr1_payment = wallet1.get_dual_address(
        TariAddressFeatures::create_interactive_only(),
        Some(payment_id.clone()),
    ).expect("Failed to generate payment address from wallet 1");
    
    let addr2_payment = wallet2.get_dual_address(
        TariAddressFeatures::create_interactive_only(),
        Some(payment_id.clone()),
...
async fn test_wallet_storage_workflow() {
    ...
    let seed_phrase = generate_seed_phrase().expect("Failed to generate seed phrase");
    let mut original_wallet = Wallet::new_from_seed_phrase(&seed_phrase, None)
    ...
    let dual_address = original_wallet.get_dual_address(
        TariAddressFeatures::create_interactive_and_one_sided(),
        None,
    ).expect("Failed to generate dual address");
    
    let single_address = original_wallet.get_single_address(
        TariAddressFeatures::create_interactive_only()
    ).expect("Failed to generate single address");
    
    // Store wallet data (simulated - in real implementation this would use the storage trait)
    let stored_seed = original_wallet.export_seed_phrase()
        .expect("Failed to export seed phrase");
    ...
    assert_eq!(original_wallet.current_key_index(), recovered_wallet.current_key_index());
    assert_eq!(original_wallet.get_property("test_key"), recovered_wallet.get_property("test_key"));
    
    // Verify addresses are consistent
    let recovered_dual = recovered_wallet.get_dual_address(
        TariAddressFeatures::create_interactive_and_one_sided(),
        None,
    ).expect("Failed to generate dual address from recovered wallet");
    
    let recovered_single = recovered_wallet.get_single_address(
        TariAddressFeatures::create_interactive_only()
...
async fn test_address_features_workflow() {
    ...
    for (network_name, expected_network) in networks {
        ...
        let interactive_dual = wallet.get_dual_address(
            TariAddressFeatures::create_interactive_only(),
            None,
        ).expect("Failed to generate interactive dual address");
        
        let interactive_single = wallet.get_single_address(
            TariAddressFeatures::create_interactive_only()
        ...
        assert!(interactive_dual.features().contains(TariAddressFeatures::INTERACTIVE_ONLY));
        assert!(interactive_single.features().contains(TariAddressFeatures::INTERACTIVE_ONLY));
        
        // Test one-sided only features
        let onesided_dual = wallet.get_dual_address(
            TariAddressFeatures::create_one_sided_only(),
            None,
        ).expect("Failed to generate one-sided dual address");
        
        let onesided_single = wallet.get_single_address(
            TariAddressFeatures::create_one_sided_only()
        ...
        let combined_dual = wallet.get_dual_address(
            TariAddressFeatures::create_interactive_and_one_sided(),
            None,
