...
fn create_test_output(
    ...
    use tari_wallet::data_structures::{
        ...
        script::TariScript,
        wallet_output::LightweightExecutionStack,
    };
...
async fn test_basic_scanning_workflow() {
    ...
    let expected_value: u64 = (100..110)
        .map(|height| 1000000 + height * 10000)
        .sum();
    ...
    println!("  Found {} wallet outputs with total value {}", total_outputs, total_value);
}
...
async fn test_scanning_with_progress() {
    ...
    println!("  Final progress: height {}, outputs {}", 
...
async fn test_wallet_scanning_workflow() {
    ...
    for height in 300..320 {
        ...
        let other_output = create_test_output(
            1000000,
            &other_view,
            &other_spend,
    ...
    // Execute wallet scan
    let start_time = Instant::now();
    let wallet_result = DefaultScanningLogic::scan_wallet_with_progress(
        &mut scanner,
        wallet_scan_config,
        None,
    ).await.expect("Wallet scan failed");
    let scan_duration = start_time.elapsed();
    ...
    let wallet_outputs: usize = wallet_result.block_results
        .iter()
    ...
    println!("  Found {} wallet outputs in {} blocks", 
             wallet_result.total_wallet_outputs, wallet_result.block_results.len());
...
async fn test_balance_calculation_workflow() {
    ...
    for (i, &value) in output_values.iter().enumerate() {
        ...
        let output = create_test_output(value, &view_key, &spend_key)
            .expect("Failed to create test output");
        scanner.add_test_block(height, vec![output]);
    }
    ...
    for result in &results {
        let height_balance: u64 = result.wallet_outputs
            .iter()
            .map(|wo| wo.value().as_u64())
            .sum();
    ...
    println!("  Total balance: {} µT ({} outputs)", total_balance, output_count);
    println!("  Mature: {} µT, Immature: {} µT", mature_balance, immature_balance);
}
...
async fn test_scanning_error_handling() {
    ...
    let empty_results = scanner.scan_blocks(empty_config).await
    ...
    let large_batch_results = scanner.scan_blocks(large_batch_config).await
    ...
    let missing_results = scanner.scan_blocks(missing_config).await
...
async fn test_concurrent_scanning() {
    ...
    let results1 = scanner.scan_blocks(config1).await
        .expect("First scan failed");
    
    let results2 = scanner.scan_blocks(config2).await
    ...
    for h1 in &heights1 {
        assert!(!heights2.contains(h1), "Height {} found in both result sets", h1);
    }
...
fn derive_test_keys(wallet: &Wallet) -> (PrivateKey, PrivateKey) {
    ...
    let spend_key = PrivateKey::from_canonical_bytes(&spend_key_bytes)
        .expect("Failed to create spend key");
