...
impl DatasetGenerator {
    ...
    fn generate_addresses(&self, wallet: &Wallet, count: usize) -> Vec<TariAddress> {
        ...
        for i in 0..count {
            ...
            let address = if i % 2 == 0 {
                wallet.get_dual_address(features, None)
                    .expect("Failed to generate dual address")
            } else {
                wallet.get_single_address(features)
    ...
    fn create_mock_output(&self, value: u64) -> LightweightTransactionOutput {
        use tari_wallet::data_structures::{
            ...
            wallet_output::{LightweightExecutionStack, LightweightOutputFeatures, LightweightOutputType},
        };
...
async fn test_large_scale_wallet_generation() {
    ...
    assert!(metrics.throughput_per_second > 100.0, 
            "Wallet generation too slow: {:.2} wallets/sec", metrics.throughput_per_second);
    
    println!("✓ Large-scale wallet generation test passed");
    println!("  Generated {} wallets in {:?}", WALLET_COUNT, generation_duration);
...
async fn test_large_scale_address_generation() {
    ...
    assert!(metrics.throughput_per_second > 500.0,
            "Address generation too slow: {:.2} addresses/sec", metrics.throughput_per_second);

    println!("✓ Large-scale address generation test passed");
    println!("  Generated {} addresses in {:?}", ADDRESS_COUNT, generation_duration);
    println!("  Distribution: {} dual, {} single", dual_count, single_count);
...
async fn test_large_dataset_scanning_performance() {
    ...
    println!("Dataset generation: {} blocks with {} outputs in {:?}",
    ...
    // Analyze results
    let total_wallet_outputs: usize = scan_results.iter()
    ...
    assert!(metrics.throughput_per_second > 1000.0,
            "Scanning too slow: {:.2} outputs/sec", metrics.throughput_per_second);

    println!("✓ Large dataset scanning test passed");
    println!("  Scanned {} outputs in {} blocks in {:?}", 
...
async fn test_concurrent_wallet_operations() {
        ...
        let handle = tokio::spawn(async move {
            ...
            for op_id in 0..OPERATIONS_PER_WALLET {
                ...
                match op_id % 4 {
                    ...
                    2 => {
                        // Seed phrase export
                        let seed = wallet.export_seed_phrase()
    ...
    for handle in handles {
        timeout(timeout_duration, handle).await
    ...
    assert!(metrics.throughput_per_second > 100.0,
            "Concurrent operations too slow: {:.2} ops/sec", metrics.throughput_per_second);

    println!("✓ Concurrent wallet operations test passed");
    println!("  {} wallets × {} operations = {} total ops in {:?}",
...
async fn test_memory_usage_stress() {
    ...
    for iteration in 0..STRESS_ITERATIONS {
        ...
        if iteration % 10 == 0 {
            println!("  Iteration {}/{}: peak delta {:.2} MB", 
                     iteration, STRESS_ITERATIONS, iteration_peak - iteration_start);
        }
    }

    let total_objects = STRESS_ITERATIONS * OBJECTS_PER_ITERATION * 2; // wallets + addresses
    let average_memory_per_iteration: f64 = peak_memory_per_iteration.iter().sum::<f64>() / peak_memory_per_iteration.len() as f64;
    ...
    assert!(max_memory_per_iteration < 100.0, "Memory usage too high: {:.2} MB", max_memory_per_iteration);
    assert!(average_memory_per_iteration < 50.0, "Average memory usage too high: {:.2} MB", average_memory_per_iteration);

    println!("✓ Memory usage stress test passed");
    println!("  {} iterations × {} objects = {} total objects", 
             STRESS_ITERATIONS, OBJECTS_PER_ITERATION * 2, total_objects);
    println!("  Average memory per iteration: {:.2} MB", average_memory_per_iteration);
    println!("  Peak memory per iteration: {:.2} MB", max_memory_per_iteration);
...
async fn test_performance_degradation() {
    ...
    let average_throughput: f64 = window_metrics.iter()
        .map(|m| m.throughput_per_second)
        .sum::<f64>() / window_metrics.len() as f64;

    // Performance assertions
    assert!(degradation_percentage < 20.0, 
            "Performance degraded too much: {:.2}%", degradation_percentage);
    assert!(average_throughput > 500.0,
            "Average throughput too low: {:.2} ops/sec", average_throughput);

    println!("✓ Performance degradation test passed");
    println!("  {} windows × {} operations = {} total", 
             TIME_WINDOWS, OPERATIONS_PER_WINDOW, TIME_WINDOWS * OPERATIONS_PER_WINDOW);
...
async fn test_concurrent_scanning_operations() {
    ...
    for scanner_id in 0..CONCURRENT_SCANNERS {
        ...
        let handle = tokio::spawn(async move {
            ...
            let total_outputs = scan_results.iter()
                .map(|r| r.outputs.len())
                .sum::<usize>();
            let wallet_outputs = scan_results.iter()
                .map(|r| r.wallet_outputs.len())
                .sum::<usize>();
    ...
    // Analyze results
    let results_guard = results.lock().await;
    let total_outputs_scanned: usize = results_guard.iter()
        .map(|(_, _, outputs, _)| *outputs)
        .sum();
    let total_wallet_outputs: usize = results_guard.iter()
        .map(|(_, _, _, wallet_outputs)| *wallet_outputs)
        .sum();

    let average_scan_duration: Duration = results_guard.iter()
        .map(|(_, duration, _, _)| *duration)
        .sum::<Duration>() / results_guard.len() as u32;
    ...
    assert!(metrics.throughput_per_second > 1000.0,
            "Concurrent scanning too slow: {:.2} outputs/sec", metrics.throughput_per_second);
